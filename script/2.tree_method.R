...
CirclePlot1 = function(tree,outname){
  ...
  tree_cell_count = cm2 %>% group_by(tags, celltype) %>% dplyr::summarise(count = sum(Freq))
  ...
  ggsave(p,filename = paste0(outname,"_circpack.pdf"))
...
TreeConstruct = function(data,bl,prefix,outname,cell){
  TagStat = function(x,blt) {
    x = as.character(x)
    t = x[2]
...
#heatmap 算一个共同节点和相邻节点的细胞相关性就好了?
{
  
  
  MyHeatmap = function(tree,outname){
    tree_cell_norm = tree$count
    tree_cell_norm[,-1] = log(tree_cell_norm[,-1]+1)
    tree_cell_norm[,-1] = t(apply(tree_cell_norm[,-1], 1, function(x) {x/sum(x)}))
    rownames(tree_cell_norm) = tree_cell_norm$tags ;tree_cell_norm = tree_cell_norm[-1]
    ...
    tree_cell_norm = tree_cell_norm[which(rowSums(tree_cell_countm)>1),]
    
    
    library('pvclust')
    
    fit2 <- pvclust(tree_cell_norm, method.hclust="ward",
                    method.dist="euclidean")
    pdf(paste0(outname,"_pvclust_tree.pdf"))
